...
class Adventure {
  ...
  val kitchen = new Area("Kitchen", "There are chefs noisily shuffling about.",Vector(None))
  val bar = new Area("Bar", "A counter behind which are two bartenders and dozens of bottles.", Vector(None))
  val wc = new Area("WC", "A nice looking area with five stalls and sinks.", Vector(None))
